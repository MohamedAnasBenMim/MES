...
def operator_dashboard(request):
    ...
    current_week_records = (
        OperatorPerformance.objects.filter(
            username__iexact=username,
            completed_at__gte=(
                week_start_datetime
            ),
    ...
    current_summary = (
        current_week_records.aggregate(
            total_points=Sum(
                "points"
            ),
    ...
    for performance in recent_performance:
        if (
            performance.qty_reject
            and performance.qty_reject > 0
        ):
