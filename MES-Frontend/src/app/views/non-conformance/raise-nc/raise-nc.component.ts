...
@Component({
  ...
  templateUrl:
    './raise-nc.component.html',
  styleUrl:
    './raise-nc.component.css',
})
export class RaiseNcComponent {
  ...
  private getMissingFields():
    ...
    if (
      ...
      this.ncSubject.trim().length
      > 30
    ) {
      missing.push(
        'NC Subject must be 30 characters or fewer'
      );
    }

    if (
      !this.productionOrder.trim()
    ) {
      missing.push(
        'Production Order No'
      );
          ...
          attr: [
            ...
            },
            {
              name:
                'MDS_EntityType',
              value:
                'InforERPEnterpriseQualityNonConformingMaterialReport',
            },
            {
              name:
                'MDS_EntityType',
