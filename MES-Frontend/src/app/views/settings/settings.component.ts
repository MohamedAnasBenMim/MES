...
export class SettingsComponent implements OnInit {
      ...
      .subscribe({
        next: (response) => {
          const savedSettings: UserSettings = {
            ...
            country: response.country || this.form.country,

            timezone: response.timezone || this.form.timezone,

            language: response.language || this.form.language,

            date_format: response.date_format || this.form.date_format,

            time_format: response.time_format || this.form.time_format,

            theme: this.normalizeTheme(response.theme || this.form.theme),
          };

          this.applyResponse(savedSettings);

          this.languageService.applyLanguage(savedSettings.language);

          this.applyTheme(this.normalizeTheme(savedSettings.theme));

          this.updateStoredUser(savedSettings);

          localStorage.setItem('country', savedSettings.country);

          localStorage.setItem('timezone', savedSettings.timezone);

          localStorage.setItem('date_format', savedSettings.date_format);

          localStorage.setItem('time_format', savedSettings.time_format);

          window.dispatchEvent(
            new CustomEvent('mes-user-settings-updated', {
              detail: savedSettings,
            }),
          );

          this.successMessage = this.t('SETTINGS_PAGE.SAVE_SUCCESS');
  ...
  private applyResponse(settings: UserSettings): void {
    this.form = {
      language: this.languageService.normalize(settings.language),

      timezone: settings.timezone || 'Africa/Tunis',

      country: settings.country || 'TN',
    ...
    localStorage.setItem('date_format', this.form.date_format);

    localStorage.setItem('time_format', this.form.time_format);
  }

  private normalizeTheme(theme: string | null | undefined): ThemePreference {
  ...
  private updateStoredUser(settings: UserSettings): void {
